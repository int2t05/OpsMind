...
func TestAPI_Ticket_PortalLifecycle(t *testing.T) {
	...
	ticket := assertOK(t, ts.doAuth(t, http.MethodGet, "/api/v1/portal/tickets?page=1&page_size=10", nil))["data"].([]interface{})[0].(map[string]interface{})
	assert.NotEmpty(t, ticket["ticket_no"])

	assertOK(t, ts.doAuth(t, http.MethodGet, fmt.Sprintf("/api/v1/portal/tickets/%d", ticketID), nil))
...
func TestAPI_Ticket_AdminFiltering(t *testing.T) {
	...
	assert.GreaterOrEqual(t, len(assertOK(t, ts.doAuth(t, http.MethodGet, "/api/v1/admin/tickets?status=1", nil))["data"].([]interface{})), 2)
}
