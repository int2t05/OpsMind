...
| 10004 | 404 | 资源不存在 |
| 10005 | 409 | 资源冲突（如账号名/角色名/配置名重复、角色有关联用户） |
...
|--------|------|
| `user:manage` | 用户与角色管理 |
| `ticket:read` | 申告查看（列表/详情） |
| `ticket:write` | 申告操作（状态变更/记录/知识候选） |
| `ticket:manage` | 申告全管理（read + write） |
| `knowledge:read` | 知识库查看 |
| `knowledge:write` | 知识库编辑（KB/文章 CRUD + 文档上传） |
| `knowledge:create` | 知识创建（从申告生成知识候选） |
| `knowledge:review` | 知识审核（审核/发布/停用/启用） |
| `knowledge:manage` | 知识全管理（read + write + review） |
| `dashboard:read` | 数据看板查看 |
| `audit:read` | 审计日志查看 |
