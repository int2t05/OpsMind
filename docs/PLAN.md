...
| 文件路径 | 验证范围 | 所属任务 |
| --- | --- | --- |
| `server/pkg/jwt/jwt_test.go` | JWT 生成/解析/过期/刷新 | T05 |
| `server/pkg/hash/hash_test.go` | bcrypt 哈希/验证/密码策略正则 | T05 |
| `server/pkg/response/response_test.go` | 统一响应格式输出 | T05 |
| `server/internal/middleware/auth_test.go` | JWT 中间件：有效令牌/过期令牌/缺失令牌 | T12 |
| `server/internal/middleware/rbac_test.go` | RBAC 中间件：有权限/无权限/管理员权限 | T13 |
| `server/internal/adapter/rag_client_test.go` | RagClient：请求构造/响应映射/超时/错误降级 | T20 |
| `server/internal/adapter/storage_client_test.go` | StorageClient：上传/预签名/删除 | T27 |
| `server/internal/service/auth_service_test.go` | 登录成功/密码错误/账号冻结/令牌刷新 | T11 |
| `server/internal/service/user_service_test.go` | 用户 CRUD/冻结恢复/密码校验/用户名重复 | T14 |
| `server/internal/service/role_service_test.go` | 角色 CRUD/菜单绑定 | T15 |
| `server/internal/service/knowledge_service_test.go` | 知识 CRUD/审核流程/发布同步/停用/重试 | T18, T19 |
| `server/internal/service/chat_service_test.go` | 问答创建/置信度判断/低置信度转人工/AI 降级/会话保存 | T26 |
| `server/internal/service/ticket_service_test.go` | 申告 CRUD/状态机转换/补充信息计数/回访/自动关闭 | T24 |
| `server/internal/service/dashboard_service_test.go` | 统计 SQL/趋势数据 | T32 |
| `server/internal/service/config_service_test.go` | 配置 CRUD/embedding 管理 | T34 |
| `server/internal/service/message_service_test.go` | 消息查询/标记已读/未读计数 | T29 |
| `server/internal/service/scheduler_test.go` | 自动关闭 7 天逻辑/消息通知触发 | T30 |
| `server/internal/handler/auth_test.go` | 登录接口集成测试 | T11 |
| `server/internal/handler/chat_test.go` | 问答接口集成测试（含降级场景） | T26 |
| `server/internal/handler/ticket_test.go` | 申告接口集成测试（含状态机） | T24 |
| `server/internal/handler/knowledge_test.go` | 知识管理接口集成测试 | T18, T19 |
| `server/internal/handler/user_test.go` | 用户管理接口集成测试 | T14 |
| `server/internal/database/database_test.go` | 数据库连接和迁移验证 | T03, T04 |
...
- Test: `server/pkg/response/response_test.go`
- Test: `server/pkg/jwt/jwt_test.go`
- Test: `server/pkg/hash/hash_test.go`
...
- Test: `server/internal/service/config_service_test.go`
...
- Test: `server/internal/middleware/rbac_test.go`
...
- Test: `server/internal/service/role_service_test.go`
...
- Test: `server/internal/service/knowledge_service_test.go`
- Test: `server/internal/handler/knowledge_test.go`
...
- Test: `server/internal/adapter/rag_client_test.go`
...
- Test: `server/internal/service/ticket_service_test.go`
- Test: `server/internal/handler/ticket_test.go`
...
- Test: `server/internal/service/chat_service_test.go`
...
- Test: `server/internal/adapter/storage_client_test.go`
...
- Test: `server/internal/service/message_service_test.go`
...
- Test: `server/internal/service/scheduler_test.go`
...
- Test: `server/internal/service/dashboard_service_test.go`
