...
- **RAG 增强智能问答** — 自建管道（查询改写→多路检索→BM25+向量混合→cross-encoder 重排序→LLM生成），SSE 流式输出
...
必须服务（4 个）:
  ...
  opsmind-web     — Next.js 前端 (Node.js)
...
| 状态管理 | React Context + SWR 2.x（服务端状态缓存） |
| 部署 | Docker Compose |
| 设计系统 | Apple Design（SF Pro / Inter Variable, 双主题 CSS 变量） |
