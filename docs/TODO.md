...
chunker(2) + parser(4) + processor(5) = 11 项 + 置信度/索引/LLM fallback/BM25 TTL/Processor pool = 5 项 + CreateSession ctx/GetChatDetail 归属/SubmitFeedback 校验/Question max/route_count/PipelineStep = 6 项
...
> 📌 标记项为代码中已存在的 TODO 注释（共 51 项，已实现 4 项移出 §1-§10）。
...
**最后更新**：2026-06-16（§2 全部 22 项修复完毕 / 置信度钳位 + 索引 + LLM fallback + BM25/Processor env 可配置；P0 20 项；总计 ~212 项）
