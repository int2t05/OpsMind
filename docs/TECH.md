...
OpsMind 采用单体分层架构（Modular Monolith），按 Handler → Service → Repository 三层分离。RAG 引擎（`rag/` 包）为自包含领域模块，不依赖 HTTP 层。
...
- **LLMService**：RAG 检索 + 动态 prompt 构建 + LLM 流式/同步调用统一编排。`StreamChat` 用于 SSE 流式路径，`SyncChat` 用于非流式 JSON 路径。ChatService 通过 LLMService 间接使用 LLMClient。
...
| BM25 检索 | 降级——仅用向量结果 | — |
| RRF 融合 | 降级——使用单路结果 | — |
| 重排序 | 降级——使用融合排序结果 | reranker == nil 时静默跳过 |
| LLM 生成 | **阻塞**——核心路径，返回错误 | — |

重排序使用 cross-encoder 模型，通过 Python 子进程 stdin/stdout JSON Lines 协议通信。模型在子进程启动时加载常驻内存，单次推理延迟约 50ms。重排前按 RerankCount 截断候选池。
...
| `OPSMIND_MINIO_ACCESS_KEY` / `OPSMIND_MINIO_SECRET_KEY` | MinIO 凭据 | `minioadmin` |
...
| 10004 | 404 | 资源不存在 |
| 10005 | 409 | 资源冲突（账号名/角色名/配置名重复、角色有关联用户） |
| 10006 | 400 | 用户已被冻结 |
| 10007 | 400 | 用户已处于正常状态 |
| 20001 | 503 | AI 服务不可用 |
| 20002 | 503 | RAG 服务不可用 |
...
`Scheduler` 每小时执行 `TicketAutoCloseJob`：

1. `Scheduler.runAutoCloseLoop` → 每小时触发
2. `TicketService.AutoClose(olderThan)` → 单事务中原子执行：
   - `TicketRepo.AutoCloseTickets(olderThan)` 原子关闭并返回 ID 列表
   - 同一事务内为每个 ticket 创建 `action="auto_close"` 的 `TicketRecord`
3. `TicketRepo.AutoCloseTickets(olderThan)` → 单 SQL：`UPDATE ... RETURNING id`

关闭条件：`status IN (1,2,3) AND created_at < now() - 7 days`

### 8.1 事务管理

`TxManager` 接口提供统一的事务抽象：
...
`TicketService` 通过构造函数注入 `TxManager`，事务编排由 Service 层完成，Repository 只做纯数据操作。

**CAS 状态转换：** 所有状态更新使用 `WHERE id=? AND status=?` 条件，确保仅在当前状态匹配时才执行更新，防止并发操作产生双重记录。
...
│   ├── handler/                    # HTTP Handler 层（11 个模块）
│   ├── service/                    # 业务逻辑层（含 tx_manager + scheduler）
│   ├── repository/                 # 数据访问层（含 pagination 辅助）
│   ├── model/                      # GORM 数据模型
│   ├── rag/                        # RAG 引擎
...
web/
├── src/
│   ├── app/                        # Next.js App Router（auth/portal/admin）
│   ├── components/
│   │   ├── ui/                     # Apple Design 原子组件
