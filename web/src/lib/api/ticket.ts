...
export function getMyTickets(page: number) { return apiFetchPage<Ticket>(`/api/v1/portal/tickets?page=${page}&page_size=10`); }
...
export function listAllTickets(page: number, status?: number, urgency?: number) {
  let url = `/api/v1/admin/tickets?page=${page}&page_size=10`;
...
export function addTicketRecord(id: number, action: string, content: string) {
  return apiFetch<null>(`/api/v1/admin/tickets/${id}/records`, { method: 'POST', body: JSON.stringify({ action, content }) });
}
