...
export const AppleInput = forwardRef<HTMLInputElement, AppleInputProps>(
  ({ pill, label, error, className = '', id, ...rest }, ref) => {
    ...
    const inputClass = [
      'w-full h-11 px-4 text-body rounded-[var(--radius-sm)] border bg-[var(--color-canvas)] text-[var(--color-ink)] outline-none transition focus:border-[var(--color-accent)] focus:shadow-[var(--focus-ring)]',
...
export const AppleTextarea = forwardRef<HTMLTextAreaElement, AppleTextareaProps>(
  ({ label, error, rows = 4, className = '', id, ...rest }, ref) => {
    ...
    const textareaClass = [
      'w-full px-4 py-3 text-body leading-relaxed rounded-[var(--radius-sm)] border bg-[var(--color-canvas)] text-[var(--color-ink)] outline-none resize-y font-sans transition focus:border-[var(--color-accent)] focus:shadow-[var(--focus-ring)]',
      error ? 'border-[var(--color-error)]' : 'border-[var(--color-hairline)]',
      className,
    ]
      .filter(Boolean)
