  ...
  return (
    <div className="max-w-content">
      <div className="flex items-center gap-3 mb-5">
        <AppleButton variant="ghost" onClick={() => router.push(`/admin/knowledge/${kbId}`)} className="p-3.5" aria-label="返回"><ArrowLeft size={15} /></AppleButton>
      </div>
      <div className="flex justify-between items-center mb-5">
        ...
        <div className="flex gap-2 flex-wrap">
          {article.status === 1 && <AppleButton onClick={() => handleAction(() => submitReview(Number(articleId)))} loading={processing}><Send size={15} /> 提交审核</AppleButton>}
          {article.status === 2 && <><AppleButton onClick={() => handleAction(() => reviewArticle(Number(articleId), true))} loading={processing}><CheckCircle size={15} /> 通过</AppleButton><AppleButton variant="ghost" onClick={() => { if (reviewComment) handleAction(() => reviewArticle(Number(articleId), false, reviewComment)); else toast.error('驳回时需填写理由'); }} loading={processing}><XCircle size={15} /> 驳回</AppleButton></>}
          {article.status === 3 && <AppleButton onClick={() => handleAction(() => publishArticle(Number(articleId)))} loading={processing}><Rocket size={15} /> 发布</AppleButton>}
          {article.status === 4 && <AppleButton variant="utility" onClick={() => setDisableConfirm(true)} loading={processing}><Pause size={15} /> 停用</AppleButton>}
          {article.status === 0 && <AppleButton onClick={() => handleAction(() => enableArticle(Number(articleId)))} loading={processing}><Play size={15} /> 启用</AppleButton>}
          {(article.status === 1 || article.status === 5) && <AppleButton variant="ghost" className="p-3.5" aria-label="编辑" onClick={startEdit}><Pencil size={15} /></AppleButton>}
        </div>
      </div>
      ...
      {editing ? (
        <AppleCard className="mb-4">
          ...
          <div className="flex gap-2"><AppleButton onClick={handleSave} loading={editSaving}><CheckCircle size={15} /> 保存</AppleButton><AppleButton variant="ghost" onClick={() => setEditing(false)}><XCircle size={15} /> 取消</AppleButton></div>
        </AppleCard>
      ) : (
        <AppleCard className="mb-4">
